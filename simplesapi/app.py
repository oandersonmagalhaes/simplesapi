...
class SimplesConfig(BaseModel):
    ...
    database_connection_string: Optional[str] = Field(
        default=settings.SIMPLESAPI_DB_CONN
    )


class SimplesAPI(FastAPI):
    def __init__(
        self, routes_path=None, redis_url=None, redis_ssl=None, *args, **kwargs
    ):
        self.simples = SimplesConfig(
            routes_path=routes_path, redis_url=redis_url, redis_ssl=redis_ssl, **kwargs
        )
